...
export default function ServicesAccordion() {
  ...
  function buildWaUrl(serviceTitle: string, itemName: string, tierLabel: string, price?: number | null): string {
    const priceText = price ? `Rp ${price.toLocaleString("id-ID")}` : t("customPrice");
    const message = t("waMessage", { service: serviceTitle, item: itemName, tier: tierLabel, price: priceText });
                ...
                <div className="flex-1">
                  ...
                  {service.techStack && (
                    <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
                      <span className="text-[10px] uppercase tracking-[0.15em] text-slate-400 font-semibold">{t("tools")}</span>
                        ...
                        {tiers.map(({ key, data, accent, dark }) => (
                          ...
                          >
                            ...
                            <div className={`text-xl sm:text-2xl font-bold tabular-nums mb-2 ${
                              ...
                              {data.price ? `Rp ${data.price.toLocaleString("id-ID")}` : t("contactUs")}
                            </div>

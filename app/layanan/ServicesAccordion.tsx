      ...
      <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimateIn className="flex flex-col items-start gap-3 mb-12 sm:mb-20">
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-900 leading-tight">
            Layanan yang dirancang untuk<br className="hidden sm:block" /> tumbuh bersama bisnis Anda.
          </h2>
          <p className="text-slate-500 leading-relaxed max-w-lg text-sm sm:text-base">
            Pilih paket yang sesuai dengan kebutuhan Anda. Setiap layanan memiliki
            3 kategori dengan harga transparan dan fitur yang berbeda.
          </p>
        </AnimateIn>

        {/* ── Services list ── */}
        <AnimateIn delay={150} className="divide-y divide-slate-100 border-t border-slate-100">
          {servicesData.map((service, index) => {
            const num = String(index + 1).padStart(2, "0");

            return (
              <div key={service.id} className="py-10 sm:py-14">

                {/* ── Service header ── */}
                <div className="flex items-start gap-4 sm:gap-6 mb-6 sm:mb-8">
                  <span className="text-[11px] text-slate-300 font-mono tabular-nums shrink-0 pt-1.5 sm:pt-2.5">
                    {num}
                  </span>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-brand mb-2 sm:mb-3">
                      {service.title}
                    </h3>
                    <p className="text-slate-500 leading-relaxed text-sm sm:text-base max-w-2xl">
                      {service.description}
                    </p>
                  </div>
                </div>
                ...
                {/* ── Body ── */}
                <div className="pl-8 sm:pl-14 space-y-8 sm:space-y-10">

...
export default function ProjectDetail({ portfolioId }: { portfolioId: number }) {
  ...
  return (
    <div className="bg-white relative mt-16 sm:mt-[72px]">
      ...
      {/* ══════════════════════════════════════
          CONTENT
      ══════════════════════════════════════ */}
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-12 lg:py-20 relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-12 lg:gap-20">

          {/* ── Left: main editorial content ── */}
          <div className="space-y-12">
            {/* Description – simple quote style */}
            <AnimateIn>
              <p className="text-slate-700 text-xl sm:text-2xl leading-relaxed font-light border-l-2 border-brand pl-5">
                {description}
              </p>
            </AnimateIn>
                  ...
                  <div>
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h2 className="text-xs font-bold uppercase tracking-[0.25em] text-slate-900 mb-2">
                          {t("challenge")}
                        </h2>
            ...
            {/* Results — simple dark card */}
            {results && (
              <AnimateIn delay={100} className="mt-12">
                <div className="relative overflow-hidden rounded-lg bg-slate-950 p-8 sm:p-10">
          ...
          {workDetails && workDetails.length > 0 && (
            <AnimateIn delay={100} className="lg:sticky lg:top-32 self-start space-y-6">

              {/* Work list */}
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.25em] text-slate-400 mb-4">
                    {t("workDoneTitle")}
                  </p>
                <ul className="space-y-3">
                  {workDetails.map((detail: any, i: number) => (
                    <li key={i} className="flex items-start gap-2.5 group">
                      <span className="mt-0.5 text-[10px] font-mono text-slate-400 shrink-0">
              ...
              <div>
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg bg-slate-900 text-white text-xs font-semibold hover:bg-brand transition-colors duration-200"
                >
      ...
      ══════════════════════════════════════ */}
      <div className="border-t border-slate-100 bg-white relative z-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div className="grid grid-cols-2 divide-x divide-slate-100">
              ...
              <Link
                ...
                className={`group relative overflow-hidden py-8 sm:py-10 flex items-center gap-4 transition-colors duration-200 hover:bg-slate-50 ${dir === "next" ? "flex-row-reverse pl-6 lg:pl-8 -mr-4 lg:-mr-6 pr-6 lg:pr-8" : "-ml-4 lg:-ml-6 pl-6 lg:pl-8 pr-6 lg:pr-8"}`}
              >
                {/* Mini image */}
                <div className="relative w-14 h-14 rounded-md overflow-hidden shrink-0 opacity-60 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <div className={`min-w-0 flex-1 ${dir === "next" ? "text-right" : ""}`}>
                  <div className={`flex items-center gap-1.5 mb-1 ${dir === 'next' ? 'justify-end' : ''}`}>
                    ...
                    <p className="text-[9px] uppercase tracking-[0.2em] text-slate-400 font-semibold">{label}</p>
                  </div>
